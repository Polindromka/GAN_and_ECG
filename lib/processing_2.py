...
@numba.njit
def substract_running_mean(ecg, window):
    rm = np.zeros(ecg.shape)
    for i in range(ecg.shape[0]):
        rm[i] = running_mean(ecg[i], window) 
    return ecg - rm
...
def cached_load(ecg_file, params):
    ...
    if cache_is_present:
        ...
        preprocessing_params = ['LOW_PASS', 'HIGH_PASS', 'SR', 'SUBTRACT_RUNNING_MEAN']
...
def preprocess(ecg, sr, params):
    ...
    if params['SUBTRACT_RUNNING_MEAN'] is not None:
        ecg = substract_running_mean(ecg, params['SUBTRACT_RUNNING_MEAN'])
